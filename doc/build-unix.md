...
This will build gsa-qt as well, if the dependencies are met.
...
These dependencies are required:
 ...
 libsodium   | Gsaling Crypto     | A modern, portable, easy to use crypto library
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~gsa/+archive/gsa).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:gsa/gsa
    sudo apt-get update
...
If you want to build gsa-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a gsa-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip gsad" to strip the debug
