...
- [Development guidelines](#development-guidelines)
    - [General PIVX Core](#general-gsa-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
([`valgrind.supp`](https://github.com/gsa-project/gsa/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_gsa
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_gsa --log_level=test_suite
$ valgrind -v --leak-check=full src/gsad -printtoconsole
```
...
# A coverage report will now be accessible at `./test_gsa.coverage/index.html`.
```
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `gsa-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `gsa-cli` and the GUI debug console use this table to determine how to
