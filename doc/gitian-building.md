...
- [Building PIVX Core](#building-gsa-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/gsa-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/gsa-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/gsa-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/gsa-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/gsa-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/gsa-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/gsa-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@gsa.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/gsa-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/gsa-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/gsa-osx-*-build.assert
```
