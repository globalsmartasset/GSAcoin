...
The build script we'll be using is contained in the PIVX github repository ([contrib/gitian-build.py](https://github.com/gsa-project/gsa/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the PIVX repository and will need to fetch this script with the following commands:
...
- Clone the gitian-builder, gitian.sigs, gsa-detached-sigs, and gsa GitHub repos.
