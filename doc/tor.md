...
versions of Tor see [Section 3](#3-automatically-listen-on-tor).*
	...
	HiddenServiceDir /var/lib/tor/gsa-service/
	HiddenServiceVersion 2
	HiddenServicePort 45328 127.0.0.1:45328
	HiddenServicePort 61472 127.0.0.1:61472

The directory can be different of course, but (both) port numbers should be equal to
your gsad's P2P listen port (45328 by default).

	-externalip=X   You can tell gsa about its publicly reachable address using
	                ...
	                /var/lib/tor/gsa-service/hostname. For connections
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./gsad -proxy=127.0.0.1:9050 -externalip=gsazj6l4cvo2fxy.onion -listen
...
for normal IPv4/IPv6 communication, use:
	...
	./gsad -onion=127.0.0.1:9050 -externalip=gsazj6l4cvo2fxy.onion -discover
...
in `torrc` config file. For cookie authentication the user running gsad must have read
access to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running gsad to the same group and setting permissions appropriately. On
Debian-based systems the user running gsad can be added to the debian-tor group,
