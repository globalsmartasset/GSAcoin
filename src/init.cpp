...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GSA specific functionality (Masternodes, Zerocoin, Budgeting) (0-1, default: %u)"), 0));
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. GlobalSmartAsset is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. GlobalSmartAsset is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //GlobalSmartAsset specific: zerocoin and spork DB's
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zGSA if already reindexed
