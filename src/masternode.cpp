...
CAmount CMasternode::GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) {
        nSubsidy = 10215000 * COIN;
    } else if (nHeight <= 500000 && nHeight >= 0) {
        nSubsidy = 4 * COIN;
    }
    else if (nHeight <= 1000000 && nHeight > 500000  ) {
        nSubsidy = 5 * COIN;
...
{
    if(nHeight <= 268000) return GetBlockValue(nHeight) * 75 / 100;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 GSA tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 GSA tx -> 1 confirmation
