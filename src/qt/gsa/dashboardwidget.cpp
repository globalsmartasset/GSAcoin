...
#include "qt/gsa/dashboardwidget.h"
#include "qt/gsa/forms/ui_dashboardwidget.h"
#include "qt/gsa/sendconfirmdialog.h"
#include "qt/gsa/txrow.h"
    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of GSA earned via Staking & Masternodes"));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-gsa");
    setCssProperty(ui->labelPiv, "text-chart-gsa");
    ...
    setCssProperty(ui->labelAmountPiv, "text-stake-gsa-disable");
	ui->labelAmountMNRewards->setText("0 GSA");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("GSA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair GSA, zGSA
...
void DashboardWidget::onChartRefreshed()
{
    ...
    if (chartData->totalPiv > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountPiv, "text-stake-gsa");
