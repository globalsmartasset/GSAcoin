...
#include "qt/gsa/defaultdialog.h"
#include "qt/gsa/settings/settingsfaqwidget.h"
...
PIVXGUI::PIVXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
    ...
    if (windowTitle.isEmpty()) {
        windowTitle = tr("GSA Core Wallet") + " || ";
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("GlobalSmartAsset client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("GlobalSmartAsset"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("GlobalSmartAsset"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
