...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizePivxAmount") || reset)
        SoftSetArg("-anonymizegsaamount", settings.value("nAnonymizePivxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gsa.cpp during shotdown to also remove the window positions
