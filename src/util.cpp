...
const char * const PIVX_CONF_FILENAME = "globalsmartasset.conf";
const char * const PIVX_PID_FILENAME = "globalsmartasset.pid";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\globalsmartassetParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\globalsmartassetParams
    // Mac: ~/Library/Application Support/globalsmartassetParams
    // Unix: ~/.globalsmartasset-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GlobalSmartAssetParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "GlobalSmartAssetParams";
#else
    // Unix
    return pathRet / ".globalsmartasset-params";
#endif
#endif
}
