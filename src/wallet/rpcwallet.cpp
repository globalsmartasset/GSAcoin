...
#include "spork.h"
#include "zgsa/deterministicmint.h"
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
                ...
                "\nReturn information about the given GSA address.\n"
                ...
                "  \"account\" : \"account\"                 (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start gsad with -deprecatedrpc=accounts. The account associated with the address, \"\" is the default account\n"
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new GSA address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"GSAaddress\"    (string) The new GSA address\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccountaddress (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccountaddress is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "\"GSAaddress\"   (string) The account GSA address.\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new GSA address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "setaccount") {
        if (request.fHelp) {
            throw std::runtime_error("setaccount (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "setaccount is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"GSAaddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"GSAaddress\"   (string, required) The GSA address to be associated with a label.\n"
    ...
    CTxDestination dest = DecodeDestination(request.params[0].get_str());
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GSA address");
...
UniValue getaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaccount (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaccount is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "getaccount \"GSAaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"GSAaddress\"  (string, required) The GSA address for account lookup.\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("getaddressesbyaccount (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getaddressesbyaccount is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "[                     (json array of string)\n"
            "  \"GSAaddress\"  (string) a GSA address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"GSAaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"GSAaddress\"  (string, required) The GSA address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in GSA to send. eg 0.1\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"GSAaddress\",     (string) The GSA address\n"
            "      amount,                 (numeric) The amount in GSA\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signmessage \"GSAaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"GSAaddress\"  (string, required) The GSA address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"GSAaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given GSAaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"GSAaddress\"  (string, required) The GSA address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in GSA received at this address.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "getreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("getreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "getreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in GSA received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    if (request.fHelp ||
        ...
        throw std::runtime_error(
            ...
            "                    To use this deprecated argument, start gsad with -deprecatedrpc=accounts."
            "2. minconf          (numeric, optional, default=1) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start gsad with -deprecatedrpc=accounts. Only include transactions confirmed at least this many times.\n"
            "3. includeWatchonly (bool, optional, default=false) DEPRECATED. This argument will be removed in v5.0.\n"
            "                    To use this deprecated argument, start gsad with -deprecatedrpc=accounts. Also include balance in watchonly addresses (see 'importaddress')\n"
            
            "\nResult:\n"
            "amount              (numeric) The total amount in GSA received for this account.\n"
...
UniValue movecmd(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("move (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "move is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of GSA to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 GSA from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 GSA from timotei to akiko with a comment\n" +
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("sendfrom (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "sendfrom is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }

    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"to GSA address\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Send an amount from an account to a GSA address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toGSAaddress\"  (string, required) The GSA address to send funds to.\n"
            "3. amount                (numeric, required) The amount in GSA. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 GSA from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "sendmany \"\" {\"address\":amount,...} ( minconf \"comment\" )\n"
            ...
            "gsad with -deprecatedrpc=accounts\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The GSA address is the key, the numeric amount in GSA is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid GSA address: ")+name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a GSA address or hex-encoded public key.\n"
            "If 'label' is specified, assign address to that label.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of GSA addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) GSA address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"label\"      (string, optional) A label to assign the addresses to.\n"

            "\nResult:\n"
            "\"GSAaddress\"  (string) A GSA address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in GSA received by the address\n"
...
UniValue listreceivedbylabel(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts") && request.strMethod == "listreceivedbyaccount") {
        if (request.fHelp) {
            throw std::runtime_error("listreceivedbyaccount (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listreceivedbyaccount is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        help_text = "listtransactions ( \"dummy\" count from includeWatchonly )\n"
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"GSAaddress\",    (string) The GSA address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in GSA. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in GSA. This is negative and only available for the \n"
...
UniValue listaccounts(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("accounts")) {
        if (request.fHelp) {
            throw std::runtime_error("listaccounts (Deprecated, will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "listaccounts is deprecated and will be removed in v5.0. To use this command, start gsad with -deprecatedrpc=accounts.");
    }
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"account\":\"accountname\",       (string) DEPRECATED. This field will be removed in v5.0. To see this deprecated field, start gsad with -deprecatedrpc=accounts. The account name associated with the transaction. Will be \"\" for the default account.\n"
            "    \"address\":\"GSAaddress\",    (string) The GSA address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in GSA. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in GSA. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in GSA\n"
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"account\" : \"accountname\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start gsad with -deprecatedrpc=accounts. The account name involved in the transaction, can be \"\" for the default account.\n"
            "      \"address\" : \"GSAaddress\",   (string) The GSA address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in GSA\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending GSAs\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending GSAs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"GSAaddress\" \"test message\"") +
...
UniValue listunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
                ...
                "3. \"addresses\"    (string) A json array of GSA addresses to filter\n"
                "    [\n"
                "      \"address\"   (string) GSA address\n"
                "      ,...\n"
                "    ]\n"
                ...
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"address\" : \"address\",  (string) the GSA address\n"
                "    \"label\" : \"label\",      (string) The associated label, or \"\" for the default label\n"
                "    \"account\" : \"account\",  (string) DEPRECATED.This field will be removed in v5.0. To see this deprecated field, start gsad with -deprecatedrpc=accounts. Backwards compatible alias for label.\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid GSA address: ") + input.get_str());
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending GSAs.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in GSA/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total GSA balance of the wallet\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in GSA\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in GSA\n"
            ...
            "  \"paytxfee\": x.xxxx                       (numeric) the transaction fee configuration, set in GSA/kB\n"
...
UniValue setstakesplitthreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 GSA each)."
            + HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in GSA).\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same GSA address\n"
...
UniValue getzerocoinbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zGSA balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zGSA balance.\n"
...
UniValue listmintedzerocoins(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nList all zGSA mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"zGSA ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zgsaTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.push_back(Pair("zGSA ID", m.hashPubcoin.GetHex()));     // PubCoin
            ...
            if (m.hashStake.IsNull()) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zgsaTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zgsaTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zGSA mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nMint the specified zGSA amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of GSA to convert to zGSA\n"
    ...
    if (!Params().IsRegTestNet())
        throw JSONRPCError(RPC_WALLET_ERROR, "zGSA minting is DISABLED");
...
UniValue spendzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2 || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in GSA.\n"
            "      \"address\": \"xxx\"         (string) GSA address or \"zerocoinmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
    ...
    return DoZgsaSpend(nAmount, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nSpend zGSA mints to a GSA address.\n" +
            HelpRequiringPassphrase() + "\n"
...
extern UniValue DoZgsaSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetmintzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzPIVTracker* zgsaTracker = pwalletMain->zgsaTracker.get();
    std::set<CMintMeta> setMints = zgsaTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const JSONRPCRequest& request)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzPIVTracker* zgsaTracker = pwalletMain->zgsaTracker.get();
    std::set<CMintMeta> setMints = zgsaTracker->ListMints(false, false, false);
    ...
    for (CZerocoinSpend spend : listUnconfirmedSpends) {
        for (auto& meta : setMints) {
            if (meta.hashSerial == GetSerialHash(spend.GetSerial())) {
                zgsaTracker->SetPubcoinNotUsed(meta.hashPubcoin);
...
UniValue exportzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.empty() || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zGSA\n"

            "\nResult:\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"id\": \"serial hash\",  (string) the mint's zGSA serial hash \n"
            ...
            "    \"v\": version,   (numeric) The version of the zGSA\n"
            "    \"k\": \"privkey\"  (string) The zGSA private key (V2+ zGSA only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzPIVTracker* zgsaTracker = pwalletMain->zgsaTracker.get();
    std::set<CMintMeta> setMints = zgsaTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() == 0)
        throw std::runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zGSA value of zerocoin mints that were added\n"
            "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zgsaTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zGSA list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzgsaseed(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "setzGSAseed \"seed\"\n"
            "\nSet the wallet's deterministic zGSA seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zGSA seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzGSAseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzGSAseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
...
UniValue getzgsaseed(const JSONRPCRequest& request)
{
    if(request.fHelp || !request.params.empty())
        throw std::runtime_error(
            "getzGSAseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zGSA seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzGSAseed", "") + HelpExampleRpc("getzGSAseed", ""));
...
UniValue generatemintlist(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zGSA seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zGSA to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zGSA to generate.\n"
...
UniValue dzgsastate(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
                "dzGSAstate\n"
                        "\nThe current state of the mintpool of the deterministic zGSA wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzGSA_count", nCount));
...
UniValue searchdzgsa(const JSONRPCRequest& request)
{
    if(request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            "searchdzGSA\n"
            "\nMake an extended search for deterministically generated zGSA that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zGSA to start with.\n"
            "2. \"range\"       (numeric) How many zGSA to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzGSA", "1, 100, 2") + HelpExampleRpc("searchdzGSA", "1, 100, 2"));
    ...
    boost::thread_group* dzgsaThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzgsaThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzgsaThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zgsaTracker->GetSerialHashes());
...
UniValue spendrawzerocoin(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 4 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) GSA address to spend to. If not specified, "
    ...
    return DoZgsaSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
