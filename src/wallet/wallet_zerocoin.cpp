...
bool CWallet::AddDeterministicSeed(const uint256& seed)
{
    ...
    if(IsCrypted()) {
        ...
        strErr = "save zGSAseed to wallet";
    }
    //the use case for this is no password set seed, mint dzGSA,
...
{
    //If this is a zapwallettx, need to read zGSA
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZPIVOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zGSA output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to GSA
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    if (res.status != CWallet::CommitStatus::OK) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            ...
            zgsaTracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zgsaTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zgsaTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zGSA was successful
        ...
        CTxDestination * changeAddress)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zGSA"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zGSA public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zGSA used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zgsaTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zGSA mints to use in this spend
    ...
    for (CZerocoinMint mint : vSelectedMints) {
        ...
        if (IsSerialInBlockchain(mint.GetSerialNumber(), nHeightSpend)) {
            ...
            if (!zgsaTracker->HasSerialHash(hashSerial))
                return error("%s: tracker does not have serialhash %s", __func__, hashSerial.GetHex());

            CMintMeta meta = zgsaTracker->Get(hashSerial);
            meta.isUsed = true;
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to GSA address to the transaction (the actual primary spend taking place)
                // TODO: check value?
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zGSA in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zgsaTracker->GetMints(true);
    ...
    return zgsaTracker->GetBalance(false, false);
}

CAmount CWallet::GetUnconfirmedZerocoinBalance() const
{
    return zgsaTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zgsaTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzPIVWallet* zwallet)
{
    ...
    zgsaTracker = std::unique_ptr<CzPIVTracker>(new CzPIVTracker(this));
}
...
{
    return zgsaTracker->HasSerial(bnSerial);
}

bool CWallet::IsMyMint(const CBigNum& bnValue) const
{
    if (zgsaTracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zgsaTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zgsaTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zgsaTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zgsaTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zgsaTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zgsaTracker->UpdateState(meta);
    } else {
