...
CzPIVWallet::CzPIVWallet(CWallet* parent)
{
    ...
    //Check for old db version of storing zGSA seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPIVSeed_deprecated(seed)) {
            ...
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPIVSeed_deprecated()) {
                    LogPrintf("%s: Updated zGSA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zGSA seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zGSA wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
void CzPIVWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zgsaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPIVWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    // Add to zGSATracker which also adds to database
    wallet->zgsaTracker->Add(dMint, true);
