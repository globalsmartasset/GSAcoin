...
class ZerocoinSpendTest(PivxTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zgsa_bal, gsa_bal):
            zgsa_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zgsa_bal)
            gsa_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], gsa_bal)
            return zgsa_bal, gsa_bal
        ...
        zgsa_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zgsa_balance, 6666)
        ...
        zgsa_balance, balance = check_balances(denom_2, zgsa_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zgsa_balance, balance = check_balances(denom_3, zgsa_balance, balance)
        ...
        _, _ = check_balances(denom_1, zgsa_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
